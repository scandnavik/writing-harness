...
**對外長文多一道**：寫給讀者看的長文（科普／客戶訊息／提案）跑檢查時加 `--public`，會多擋一類「對外工程黑話」——像「機械可檢」「固化」這種工程圈用語，對一般讀者是術語牆（完整清單見 glossary §3.3）。對內筆記用這些詞沒問題，所以平常不擋、只有 `--public` 才紅燈。

**陸用詞的三個工程例外**：`質量`（物理 mass）、`閉環`（工程 closed-loop 控制）、`顆粒度`（技術 granularity）在正確的工程／物理語境被擋下時，人工確認語境無誤即可放行，並在留證裡註明「工程例外：詞（語境）」。其餘陸用詞一律改台用，不得援例。
