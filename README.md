...
防 AI Slop 的一套寫作品質閘。把「會不會寫成 AI 腔」這件事，從事後憑感覺修，前移成動筆前、草稿後兩個固定檢查點。方法論加腳本加 Claude Code hook，別人 clone 下來就能套用。
...
| | `hooks/output-tier-gate.py` | 對外產出自動提醒附 evidence pack |
...
安裝腳本會把方法論、腳本、hook、範本複製到 `~/.claude/skills/writing-harness/`，把 tighten skill 複製到 `~/.claude/skills/tighten/`，並跑一次煙霧測試。它**不會動你的 settings.json**，hook 接線要你自己來（見下）。
...
checklist 是給人看的承諾，harness 是會執行的系統。差別有三：機械閘是程式不是承諾，判斷閘要留證可稽核，而且每次被真人改稿揪出的新壞味道，先問「正則抓得到嗎」，抓得到就往 S1 腳本補一條，抓不到才寫成 S2 的判斷項。規則庫單向長大，品質地板只升不降。
...
├─ examples/           語感範本 + bloat 全命中/乾淨 fixture
├─ tests/              煙霧測試
