...
歡迎貢獻。請先理解這套系統唯一的設計信條：**規則庫只升不降（棘輪）。** 每抓到一個新的壞味道，就把它固化成一道閘，下次再也漏不掉。
...
- **抓得到（機械可檢）** → 加進 `scripts/taiwan-style-check.py`（或 `verbosity-check.py`）的常數區，配一個 check function。這是首選路徑。
...
3. 在 `examples/bloated-sample.md` 補一行會命中新規則的內容，確認 fixture 仍會 fail。
